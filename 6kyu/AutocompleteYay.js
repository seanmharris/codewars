...
function autocomplete(input, dictionary){
    function cleanup(string) {
      return [...string].filter(char => char.toLowerCase().charCodeAt() > 96 && char.toLowerCase().charCodeAt() < 123).join('')
    }
    
    let test = cleanup(input), len = test.length
    
    return dictionary.filter(option => option.toLowerCase().slice(0, len) === test).slice(0, 5)
  }