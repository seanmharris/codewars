...
or null integer (represented as strings) with one or two digits. Substrings in the input string are separated by ,  or ,.
...
Median : In statistics, the median is the number separating the higher half of a data sample from the lower half. 
    ...
    If there is an even number of observations, then there is no single middle value; the median is then defined to be the mean of
...
function stat(strg) {
    let secondTotals = []
    let arr = strg.split(', ')
    let subarr = arr.map(time => time.split('|'))
    for (let arr of subarr) {
      let [hour, min, sec] = arr
      secondTotals.push((+hour * 3600) + (+min * 60) + +sec)
    }
    secondTotals.sort((a,b) => a - b);
  
    const range = secondTotals[secondTotals.length - 1] - secondTotals[0]
    const average = Math.trunc(secondTotals.reduce((a,b) => a + +b, 0) / secondTotals.length)
    let median = 0;
    if (secondTotals.length % 2 === 0) {
      median = Math.trunc((secondTotals[secondTotals.length / 2] + secondTotals[(secondTotals.length / 2) - 1]) / 2)
    } else median = secondTotals[Math.floor(secondTotals.length / 2)]
    
    return `Range: ${reSegment(range)} Average: ${reSegment(average)} Median: ${reSegment(median)}`
    
    function reSegment(seconds) {
