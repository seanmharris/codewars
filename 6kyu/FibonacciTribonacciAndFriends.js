...
a Pentabonacci (well Cinquebonacci would probably sound a bit more italian, but it would also sound really awful) with a signature
...
function Xbonacci(signature,n){
    ...
    if (n < num) return signature.slice(0,n)
    let cur = 0;
    while(signature.length < n) {
      signature.push(signature.slice(cur, cur + num).reduce((a,b) => a + b))
      cur ++
    }
    return signature
  }