...
In other words, given as input a precision of epsilon we will stop the iterative process when the absolute value of the difference between our calculation using
...
function iterPi(epsilon) {
    let count = 0, overFour = 0;
    while(Math.abs(Math.PI - (4 * overFour)) > epsilon) {
      if(count % 2 === 0) {
        overFour += 1 / (2 * count + 1)
      } else {
        overFour -= 1 / (2 * count + 1)
      }
      count++
    }
    return [count, +(4 * overFour).toFixed(10)]
  }