/*
"The Shell Game" involves cups upturned on a playing surface, with a ball placed underneath one of them. The index of the cups are swapped around multiple times.
...
function findTheBall(start, swaps) {
    for (let swap of swaps) {
      if(start === swap[0]) {
        start = swap[1]
      } else if (start === swap[1]) start = swap[0]
    }
    return start
  }