...
international distress signal SOS (that was first issued by Titanic), that is coded as ···−−−···. 
...
decodeMorse = function(morseCode){
    let letters = []
    let words = morseCode.split('   ').filter(word => word != '')
    words.forEach(word => {
      letters.push(word.split(' '))
    })
    let decoded = letters.map(word => word.map(letter => letter = MORSE_CODE[letter]))
    return(decoded.join(' ').replaceAll(',',''))
  }