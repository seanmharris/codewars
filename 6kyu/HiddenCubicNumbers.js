...
function isSumOfCubes(s){
    const cubes = s.match(/\d{1,3}/g).filter(num => [...num].reduce((a,b) => a + Math.pow(+b, 3), 0) == num).map(Number);
    return cubes.length ? `${cubes.join(' ')} ${cubes.reduce((c,d) => c + +d, 0)} Lucky` : 'Unlucky';
  }