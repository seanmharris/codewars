...
|  *  | |  0  | |  #  |
------- ------- -------
Prior to the development of T9 (predictive text entry) systems, the method to type words was called "multi-tap" and involved pressing
...
function presses(phrase) {
    const library = ['adgjmptw1*# ', 'behknqux0', 'cfilorvy','234568sz', '79']
    return phrase.toLowerCase().split('').reduce((a, b) => a + finder(b, library), 0)
  }
                                     
  const finder = (character, array) => {
   for (let group of array) {
     if (group.includes(character)) return array.indexOf(group) + 1
   }   
  }