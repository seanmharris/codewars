...
function addUsername(list) {
    ...
    const createUserName = person => {
      const name = person.firstName.toLowerCase(),
            init = person.lastName[0].toLowerCase(),
            year = new Date().getFullYear() - person.age;
      return `${name}${init}${year}`
    }
    
    for (let attendee of list) attendee.username = createUserName(attendee)
