...
// global scope function to ensure only integers with DRYer syntax (num(x) is better to type in each method definition than Number.isInteger(x) imo)

function num(x) {return Number.isInteger(x)};

Array.prototype.even = function(){
  return this.filter(n => num(n) && !(n%2))
}
...
Array.prototype.under = function(x){
  return this.filter(n => num(n) && n < x)
...
Array.prototype.inRange = function(min,max){
  return this.filter(n => num(n) && n >= min && n <= max)
}
