...
function computeDepth (x){
    let digits = [], count = 1, cur;
    const isolateDigits = number => number.toString().split('').map(char => +char)
    
    while (digits.length < 10) {
      cur = x * count
      for (let digit of isolateDigits(cur)) {
        if (!digits.includes(digit)) digits.push(digit)
      }
      if(digits.length === 10) return count
      count ++
    }
    return count 
  }