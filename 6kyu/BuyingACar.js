...
function nbMonths(old, cost, acc, dep){
    if (old >= cost) return [0, old - cost]
    let months = 0
    let totalFunds = old + (months * acc)
    while(totalFunds < cost) {
      months ++
      if(months % 2 === 0) dep += 0.5
      old *= (100 - dep) / 100
      cost *= (100 - dep) / 100
      totalFunds = old + (months * acc)
    }
    return [months, Math.round(totalFunds - cost)]
  }