...
Jenny is a 3rd grader student, so when a new mission comes up, she gets a code to decipher in a form of a sticker (with numbers) in
...
function missingWord(nums, str) {
    const cipher = str.replaceAll(' ', '').toLowerCase()
    if (nums.some(num => num >= cipher.length)) return 'No mission today'
    return nums.sort((a,b) => a - b).reduce((x,y) => x + cipher[y],'')
  }