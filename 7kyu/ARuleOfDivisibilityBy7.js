...
number has at most 2 digits because you are supposed to know if a number of at most 2 digits is divisible by 7 or not.
...
function seven(m, count = 1) {
    if (m.toString().length <= 2) return [m, 0]
    
    let x = Math.floor(m / 10), y = m % 10, z = x - (2*y)
    if (z.toString().length <= 2) return [z, count]
