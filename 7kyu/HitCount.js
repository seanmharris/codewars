...
Instead of just displaying the count value immediatley when the page loads, we want to create the effect of each digit cycling through its preceding
...
function counterEffect(hitCount) {
    const digitize = num => new Array(num + 1).fill(0).map((_,i) => i)
    
    return hitCount.split('').map(e => digitize(+e)) 
  }