...
function bitsBattle(numbers) {
    let oddScore = numbers.filter(num => num % 2).map(odd => [...odd.toString(2)].filter(dig => dig === '1').length).reduce((a,b) => a + b, 0),
        evenScore = numbers.filter(num => !(num % 2) && num !== 0).map(even => [...even.toString(2)].filter(dig => dig === '0').length).reduce((c,d) => c + d, 0);
    return oddScore > evenScore ? 'odds win' : evenScore > oddScore ? 'evens win' : 'tie'
  }