...
We need a function findNextPower that receives two arguments, a value val, and the exponent of the power, pow_, and outputs the value that we want to find.
Let'see some cases:
...
function findNextPower(val, pow_) {
    const root = Math.round(Math.pow(val, (1/pow_)) * 1000)/ 1000,
          next = !(root % 1),
          base = next ? root + 1 : Math.ceil(root)
