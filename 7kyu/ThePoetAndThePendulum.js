/*
Scenario: the rhythm of beautiful musical notes is drawing a Pendulum. Beautiful musical notes are the Numbers
Task: Given an array/list [] of n integers, Arrange them in a way similar to the to-and-fro movement of a Pendulum
The Smallest element of the list of integers, must come in center position of array/list. The Higher than smallest, goes to the right.
The Next higher number goes to the left of minimum number and So on, in a to-and-fro manner similar to that of a Pendulum.
Notes: Array/list size is at least 3. In Even array size, The minimum element should be moved to (n-1)/2 index (considering that indexes start from 0)
...
function pendulum(values) {
    values.sort((a,b) => b - a)
    let resRight = [values.pop()],
        resLeft = []
    while(values.length) {
      resRight.push(values.pop())
      if(values.length) resLeft.push(values.pop())
    }
    return [...resLeft.reverse(), ...resRight]
  }