...
function toCurrency(price, str = '') {
    let pre = Math.floor(price / 1000),
        post = String(price % 1000).padStart(3,'0')
    return pre > 999 ? toCurrency(pre, ',' + post + str) :pre ? `${pre},${post}` + str : post + str
  }