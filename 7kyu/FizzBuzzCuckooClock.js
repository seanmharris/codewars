/*
You've always loved both Fizz Buzz katas and cuckoo clocks, and when you walked by a garage sale and saw an
...
say the numbers ala Fizz Buzz, but then you decided at least for version 1.0 to just have the clock make a quiet, subtle "tick"
...
needs to make at that time, separated by spaces. Note that although the input is in 24-hour time, cuckoo clocks' cuckoos are in 12-hour time.
Some examples
"13:34"       "tick"
...
function fizzBuzzCuckooClock(time) {
    const [h, m] = time.split(':'), cuckoo = 'Cuckoo '
    let sound = ''
    if (m === '00') sound = cuckoo.repeat(h % 12 ? h % 12 : 12)
    if (m === '30') sound = cuckoo
    if (sound) return sound.trim()
    
    if (!(m % 3)) sound += 'Fizz'
    if (!(m % 5)) sound += ' Buzz'
    return sound.trim() || 'tick'
  }
