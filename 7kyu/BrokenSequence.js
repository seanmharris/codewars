...
function findMissingNumber(sequence){
    ...
    // split on space and non-number input edge case
    let arr = sequence.split(' ')
    if (arr.some(el => isNaN(+el))) return 1;
    // change from strings to numbers & sort result
    let sorted = arr.map(el => +el).sort((a,b) => a - b)
    // if at any point the ordered element is not equal to index + 1, that's the break
    for (let i = 0; i < sorted.length; i++) {
      if (sorted[i] !== i + 1) return i + 1
    }
    // otherwise (success) return 0
    return 0
  }