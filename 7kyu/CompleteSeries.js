...
function completeSeries(arr) {
    ...
    return Array.from({length: Math.max(...arr) + 1}, (_,i) => i)
}

/* this also works, constructor below vs. Array.from() method with length property destructured above

function completeSeries(arr) {
    if(new Set(arr).size !== arr.length) return [0]
    return new Array(Math.max(...arr) + 1).fill(0).map((e,i) => i)
}
