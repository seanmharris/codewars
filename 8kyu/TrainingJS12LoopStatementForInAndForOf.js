...
If we want to traverse an object, we can use a variant of the for: for..in. This can traverse all the keys of the object.
An example:
...
function giveMeFive(obj){
    let res = []
    for (let item in obj) {
      if (item.length === 5) res.push(item)
      if (obj[item].length === 5) res.push(obj[item])
    }
    return res
  }