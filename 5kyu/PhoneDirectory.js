...
function phone(strng, num) {
    const arr = strng.split('\n').filter(entry => entry.includes(num)),
          err = 'Error =>'
    if (arr.length > 1) return `${err} Too many people: ${num}`
    if (arr.length < 1) return `${err} Not found: ${num}`
    let contact = arr[0],
        nameStart = contact.indexOf('<') + 1,
        nameEnd = contact.indexOf('>'),
        name = contact.slice(nameStart, nameEnd);
    contact = contact.replace(name, '')
              .replace(num, '')
              .replace(/[^A-Za-z\d\s-.]/g, ' ')
              .replace(/ {2,}/g, ' ')
    return `Phone => ${num}, Name => ${name}, Address => ${contact.trim()}` 
}